...
import { ChevronLeftIcon, ChevronRightIcon, ShoppingCartIcon, CheckCircleIcon, TagIcon, UserCircleIcon, InformationCircleIcon, CogIcon } from '@heroicons/react/24/solid';
...
const StoreItemDetailsPage: React.FC = () => {
  ...
  const cartItemCount = useCartStore(state => state.getItemCount()); // For header update (indirectly)

  useEffect(() => {
    setIsLoading(true);
    // Simulate API call
    setTimeout(() => {
      const foundItem = mockStoreItems.find(i => i.id === itemId);
  ...
  const handleQuantityChange = (newQuantity: number) => {
    if (!item) return;
    let maxQuantity = item.stock === undefined || item.stock === 0 ? 1 : item.stock; // if stock is 0, allow 1 (might be error in data), else cap by stock
    if (item.condition !== 'New' && item.stock === undefined) maxQuantity = 1; // Used items typically stock 1
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-12">
        ...
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3 pt-4">
            ...
            <Button variant="secondary" size="lg" className="w-full sm:w-auto" onClick={() => alert('Buy Now (Mock Functionality)')} disabled={itemStock === 0}>
              Buy Now
            </Button>
          </div>
        </div>
