...
import { ROUTE_PATHS, APP_LOGO_TEXT, APP_LOGO_URL } from '../../constants';
import { UserIcon, BellIcon, VideoCameraIcon, MagnifyingGlassIcon, ArrowLeftOnRectangleIcon, CogIcon, ShoppingBagIcon, XMarkIcon } from '@heroicons/react/24/outline';
...
const Header: React.FC<HeaderProps> = ({ onLogout, userId }) => {
  ...
  const profileImageUrl = userId ? `https://picsum.photos/seed/${userId}/40/40` : `https://picsum.photos/seed/placeholderUser/40/40`;
  const profileLinkPath = userId ? `${ROUTE_PATHS.PROFILE}/${userId}` : ROUTE_PATHS.LOGIN;
  const myClimbsLinkPath = userId ? ROUTE_PATHS.USER_VIDEOS(userId) : ROUTE_PATHS.LOGIN;
          ...
          {isAuthenticated ? (
            <>
              ...
              <Link to={ROUTE_PATHS.NOTIFICATIONS} title="Notifications" className="relative hover:text-primary transition-colors p-1">
                ...
                {/* Notification count should come from API, for now it's static or hidden */}
