...
import { DifficultyGrade, Gym, VideoPost, StoreItem, isGym, isVideoPost, isStoreItem } from '../../types';
...
interface AdvancedSearchFormProps {
  initialQuery?: string;
  onSearchSubmit: () => void; // Callback to close dropdown, etc.
}
...
const AdvancedSearchForm: React.FC<AdvancedSearchFormProps> = ({ initialQuery = '', onSearchSubmit }) => {
  ...
  const uniqueLocations = React.useMemo(() => Array.from(new Set(mockGyms.map(gym => gym.location))), []);
    ...
    <form onSubmit={handleSubmit} className="space-y-4 text-sm">
      ...
      {(filters.searchType === 'all' || filters.searchType === 'gyms' || filters.searchType === 'videos') && (
        <div>
          ...
          <select name="location" id="adv-location" value={filters.location} onChange={handleInputChange} className="w-full p-2 border border-slate-300 rounded-md focus:ring-primary focus:border-primary text-sm">
